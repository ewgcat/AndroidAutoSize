package com.lishuaihua.autosize.external
...
class ExternalAdaptInfo : Parcelable {
    ...
    override fun writeToParcel(dest: Parcel, flags: Int) {
        dest.writeByte(if (isBaseOnWidth) 1.toByte() else 0.toByte())
        dest.writeFloat(sizeInDp)
    }

    protected constructor(`in`: Parcel) {
        isBaseOnWidth = `in`.readByte().toInt() != 0
        sizeInDp = `in`.readFloat()
    }
